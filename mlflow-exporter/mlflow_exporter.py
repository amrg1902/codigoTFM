...
def mostrar_experimentos():
    ...
    # Itera sobre los experimentos
    for nombre_experimento in experimentos:
        # Obtén el ID del experimento por su nombre
        experimento_id = mlflow.get_experiment_by_name(nombre_experimento).experiment_id
        ...
        for index, run in runs.iterrows():
            ...
            metrics = mlflow.get_run(run_id).data.metrics
            for metric_name, metric_value in metrics.items():
                metricas_prometheus += f'{metric_name}{{experiment="{nombre_experimento}", run_id="{run_id}"}} {metric_value}\n'
            print(f"Metrics for run {run_id} in experiment {nombre_experimento}: {metrics}")
