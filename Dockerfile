FROM ubuntu:22.04 #Utilidades
RUN apt−get RUN apt−get RUN apt−get
#Python
RUN apt−get
update
install −y wget install −y libpq−dev
−y install python3 # Usuario sin privilegios
...
ENV USER=devteam
ENV HOME=/home/${USER}
# Recomendadión para designar usuarios sin privilegios (UID, GID) ENV UID 1000
ENV GID 100
# Crea el usuario sin contraseña ,
# fuera de la lista de sudoers con los parametros anteriormente
# definidos
RUN adduser −−disabled−password \ −−gecos "Non−root user" \ −−home $ {HOME} \
−−uid ${UID} \
−−gid ${GID} \
$ {USER}
# Copiar ficheros necesarios COPY entrypoint.sh ${HOME}
RUN chown ${UID}:${GID} ${HOME}/entrypoint.sh && \ chmod 0755 ${HOME}/entrypoint.sh
COPY .bashrc ${HOME}
RUN chown ${UID}:${GID} ${HOME}/.bashrc RUN mkdir /home/MLFLOWSRUN chmod 0770 /home/MLFLOWS
RUN chown devteam:users /home/MLFLOWS
# Conda
USER ${USER}
ENV CONDA_DIR=${HOME}/conda RUN wget −−quiet \
...
RUN conda RUN conda RUN conda RUN conda
update −−name base −−channel defaults conda create −n $ENV_PREFIX
clean −− a l l −−yes
activate ${ENV_PREFIX} && \
python3 −m pip install −−upgrade pip && \ pip install mlflow && \
